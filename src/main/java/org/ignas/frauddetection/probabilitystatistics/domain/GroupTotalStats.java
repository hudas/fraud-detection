...
//        Map is not really needed for functional purposes, however search by code in hashmap will be faster than iterating over the list.
        ...
        sumOfOccurencesInFraud += difference;
        sumOfSquaredFraudOccurences += squareDifference;

        averageProbability = ((float) sumOfOccurencesInFraud) /(totalTransactions * numberOfPossibleCombinations);

        float firstComposite = averageProbability * averageProbability * numberOfPossibleCombinations;
        float secondComposite = 2 * averageProbability * sumOfOccurencesInFraud / totalTransactions;
        float thirdComposite = ((float) sumOfSquaredFraudOccurences) / (totalTransactions * totalTransactions);
